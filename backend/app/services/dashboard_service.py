...
class DashboardService:
    ...
    def get_summary(self) -> DashboardSummaryResponse:
        ...
        for a in analyses.values():
            topic_sentiments[a.topic.primary_topic].append(a.sentiment.score)
            crit = sum(1 for c in a.concerns if c.severity in ["Critical", "High"])
            topic_critical_concerns[a.topic.primary_topic] += crit
        ...
        for ins in insights[:4]:  # Top priority alerts
            priority_alerts.append(PriorityAlert(
                ...
                affected_stakeholders=ins.affected_stakeholders,
                suggested_action=ins.suggestion_summary
            ))

        # 5. Aggregate Actionable counts
        total_suggestions = sum(len(a.suggestions) for a in analyses.values())
        critical_frictions = sum(1 for a in analyses.values() for c in a.concerns if c.severity == "Critical")
